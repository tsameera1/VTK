...
public:
  ...
  void Build( vtkCellTreeLocator::CellTree& ct, vtkDataSet* ds )
  {
...
typedef std::pair<vtkCellTreeLocator::CellTreeNode*, boxlevel> nodePair;
typedef std::stack<nodePair, std::vector<nodePair> > nodepairstack;
...
void vtkCellTreeLocator::GenerateRepresentation(int level, vtkPolyData *pd)
{
  ...
  ns.push(nodePair(n0, boxlevel(rootbox,0)));
  while (!ns.empty())
    {
    n0 = ns.top().first;
    int lev = ns.top().second.second;
    if (n0->IsLeaf() && ((lev==level) || (level==-1))) {
      bl.push_back(ns.top().second);
      ns.pop();
    }
    else if (n0->IsLeaf()) {
      ns.pop();
      ...
      ns.pop();
      ns.push(nodePair(n1, boxlevel(lbox,lev+1)));
      ns.push(nodePair(n2, boxlevel(rbox,lev+1)));
    }
  }
  //
  //
